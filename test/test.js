...
describe("convert to gengou", function () {
  ...
  it("calculate gengou from year, month and day", function () {
    expect(kanjidate.toGengou(1857, 9, 12)).to.eql({
      gengou: "西暦",
      nen: 1857,
    });
  });
...
describe("convert from gengou", function () {
  ...
  it("calculate year from gengou and nen", function () {
    expect(function () {
      return kanjidate.fromGengou("元号", 32);
    }).to.throw("Invalid gengou: 元号");
  });
  it("calculate year from gengou and nen", function () {
    expect(function () {
      return kanjidate.fromGengou("平成", -3);
    }).to.throw(Error);
  });
});
...
describe("format date with default format", function () {
  ...
  it("format date represented by string with default format", function () {
    expect(kanjidate.format("2016-06-14 13:04:21")).equal(
      "平成28年6月14日（火）"
    );
  });
});
...
describe("format date with format string", function () {
  it("format Date (1)", function () {
    expect(
      kanjidate.format("{G}{N}年{M}月{D}日（{W}）", new Date(2016, 6 - 1, 14))
    ).equal("平成28年6月14日（火）");
  });
  it("format Date (2)", function () {
    expect(kanjidate.format("{G}{N}年{M}月{D}日（{W}）", "2016-06-14")).equal(
      "平成28年6月14日（火）"
    );
  });
  it("format Date (3)", function () {
    expect(kanjidate.format("{G}{N}年{M}月{D}日（{W}）", 2016, 6, 14)).equal(
      "平成28年6月14日（火）"
    );
  });
});
...
describe("format datetime", function () {
  it("plain", function () {
    expect(
      kanjidate.format(
        "{G}{N}年{M}月{D}日（{W}） {a}{h:12}時{m}分{s}秒",
        2016,
        6,
        14,
        8,
  ...
  it("time in alphabet", function () {
    expect(
      kanjidate.format(
        "{G}{N}年{M}月{D}日（{W}） {h:12}:{m}:{s}{a:am/pm}",
        2016,
        6,
        14,
        19,
...
describe("predefined formatting", function () {
  it("format1", function () {
    expect(kanjidate.format(kanjidate.f1, 2016, 6, 12)).equal(
      "平成28年6月12日（日）"
    );
  });
  it("format2", function () {
    expect(kanjidate.format(kanjidate.f2, new Date(2016, 6 - 1, 12))).equal(
      "平成28年6月12日"
    );
  });
  it("format3", function () {
  ...
  it("format4", function () {
    expect(kanjidate.format(kanjidate.f4, 2016, 6, 12)).equal(
      "平成28年06月12日（日）"
    );
  });
  it("format5", function () {
    expect(kanjidate.format(kanjidate.f5, 2016, 6, 12)).equal(
      "平成28年06月12日"
    );
  });
  it("format6", function () {
  ...
  it("format7", function () {
    expect(kanjidate.format(kanjidate.f7, 2016, 6, 12, 14, 26, 8)).equal(
      "平成28年6月12日（日） 午後2時26分8秒"
    );
  });
  it("format8", function () {
    expect(kanjidate.format(kanjidate.f8, 2016, 6, 12, 14, 26, 8)).equal(
      "平成28年06月12日（日） 午後02時26分08秒"
    );
  });
  it("format9", function () {
    expect(kanjidate.format(kanjidate.f9, 2016, 6, 12, 14, 26, 8)).equal(
      "平成28年6月12日（日） 午後2時26分"
    );
  });
  it("format10", function () {
    expect(kanjidate.format(kanjidate.f10, 2016, 6, 12, 14, 26, 8)).equal(
      "平成28年06月12日（日） 午後02時26分"
    );
  });
  it("format11", function () {
    expect(kanjidate.format(kanjidate.f11, 2016, 6, 12, 14, 26, 8)).equal(
      "平成２８年６月１２日"
    );
  });
  it("format12", function () {
    expect(kanjidate.format(kanjidate.f12, 2016, 6, 12, 14, 26, 8)).equal(
      "平成２８年０６月１２日"
    );
  });
});
...
describe("format to SqlDate, SqlDateTime", function () {
  it("format13", function () {
    expect(kanjidate.format(kanjidate.f13, "2016-06-20 23:24:12")).equal(
      "2016-06-20"
    );
  });
  it("fSqlDate", function () {
    expect(kanjidate.format(kanjidate.fSqlDate, "2016-06-20 23:24:12")).equal(
      "2016-06-20"
    );
  });
  it("format14", function () {
    expect(kanjidate.format(kanjidate.f14, "2016-06-20 23:24:12")).equal(
      "2016-06-20 23:24:12"
    );
  });
  it("fSqlDateTime", function () {
    expect(
      kanjidate.format(kanjidate.fSqlDateTime, "2016-06-20 23:24:12")
...
describe("GengouList", () => {
  it("should include 令和 in GengouList", () => {
    expect(kanjidate.GengouList.map((g) => g.kanji)).includes("令和");
  });
});
